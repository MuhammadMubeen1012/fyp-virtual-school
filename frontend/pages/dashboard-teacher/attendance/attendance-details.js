...
import {getClassrooms} from "../../../components/Controllers/DashboardController";
...
const AttendanceDetail = () => {
    ...
    useEffect(() => {
        if (router.isReady) {
            ...
            getCourseAttendance(router.query.courseId).then((res) => {
                ...
                setLength(res.attendance.length);
            });

            getClassrooms([router.query.classroomId]).then((res) => {
                console.log(res);
                setClassroom(res[0]);
            });
        }
    ...
    return (
        <LayoutTeacher>
            {/*=============== Start of main ================= */}
            <main>
                <h1>Attendance {classroomLoading?classroom.name:""}</h1>
                            ...
                            ? attendance.map((item, idx) => (
                                <tr key={idx}>
                                    ...
                                    <Link
                                        style={{color: "#fff"}}
                                        href={{
                                            pathname: `/dashboard-teacher/attendance/view-attendance`,
                                            query: {
